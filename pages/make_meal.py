...
from .Shiny_layouts.Shiny_hidden_layouts import (
    cnf_layout, cnf_totals_layout, rdi_layout, rdi_totals_layout
)
...
controls_layout = dbc.Container([
    dbc.Row([
        dbc.Col([
            html.Div(  # todo: hide dataframes as json
                id="hidden-conversions-df", style={'display': 'none'}
            ),
            dcc.Store(
                id="hidden-nutrients-df", storage_type='session'
            ),
            html.Div(
                id="hidden-cumul-ingreds-df", style={'display': "none"}
            ),
            dcc.Store(
                id='hidden-total-nutrients-df', storage_type='session'

            ),
            html.Div(  # todo: trigger on selecting age and lifestage group
                id="hidden-rdi-df", style={'display': "none"}
            ),
            html.Br(),
    ...
    dbc.Row([
        dbc.Col([
            dcc.RadioItems(
                ...
                options=[
                    {'label': 'nutrient table for one', 'value': 'cnf-table'},
                    {'label': 'vs RDI for one', 'value': 'rdi-graphs'},
                    {'label': 'nutrient tables for all', 'value': 'cnf-totals-table'},
                    {'label': 'vs RDI for all', 'value': 'rdi-totals-graphs'},
                ],
                value='cnf-table'
            )
        ], width=12)
...
def layout():
    full_layout = html.Div([controls_layout, cnf_layout, rdi_layout, cnf_totals_layout, rdi_totals_layout])
